...
(function(factory) {
  ...
  var Multiselect = (function($) {
    ...
    function Multiselect($select, settings) {
      ...
      this.$left = $select;
      this.$right = $(settings.right).length
        ? $(settings.right)
        : $("#" + id + "_to");
      this.actions = {
        $leftAll: $(settings.leftAll).length
          ? $(settings.leftAll)
          : $("#" + id + "_leftAll"),
        $rightAll: $(settings.rightAll).length
          ? $(settings.rightAll)
          : $("#" + id + "_rightAll"),
        $leftSelected: $(settings.leftSelected).length
          ? $(settings.leftSelected)
          : $("#" + id + "_leftSelected"),
        $rightSelected: $(settings.rightSelected).length
          ? $(settings.rightSelected)
          : $("#" + id + "_rightSelected"),

        $undo: $(settings.undo).length
          ? $(settings.undo)
          : $("#" + id + "_undo"),
        $redo: $(settings.redo).length
          ? $(settings.redo)
          : $("#" + id + "_redo"),

        $moveUp: $(settings.moveUp).length
          ? $(settings.moveUp)
          : $("#" + id + "_move_up"),
        $moveDown: $(settings.moveDown).length
          ? $(settings.moveDown)
          : $("#" + id + "_move_down")
      };
      ...
      this.options = {
        ...
        submitAllLeft:
          settings.submitAllLeft !== undefined ? settings.submitAllLeft : true,
        submitAllRight:
          settings.submitAllRight !== undefined
            ? settings.submitAllRight
            : true,
        search: settings.search,
        ignoreDisabled:
          settings.ignoreDisabled !== undefined
            ? settings.ignoreDisabled
            : false,
        matchOptgroupBy:
          settings.matchOptgroupBy !== undefined
            ? settings.matchOptgroupBy
            : "label"
      };

      delete settings.keepRenderingSort,
        settings.submitAllLeft,
        settings.submitAllRight,
        settings.search,
        settings.ignoreDisabled,
        settings.matchOptgroupBy;
    ...
    Multiselect.prototype = {
      ...
      events: function() {
        ...
        if (self.options.search && self.options.search.$left) {
          self.options.search.$left.on("keyup", function(e) {
            if (self.callbacks.fireSearch(this.value)) {
              var $toShow = self.$left
                .find('option:search("' + this.value + '")')
                .mShow();
              var $toHide = self.$left
                .find('option:not(:search("' + this.value + '"))')
                .mHide();
              var $grpHide = self.$left
                .find("option")
                .closest("optgroup")
                .mHide();
              var $grpShow = self.$left
        ...
        if (self.options.search && self.options.search.$right) {
          self.options.search.$right.on("keyup", function(e) {
            if (self.callbacks.fireSearch(this.value)) {
              var $toShow = self.$right
                .find('option:search("' + this.value + '")')
                .mShow();
              var $toHide = self.$right
                .find('option:not(:search("' + this.value + '"))')
                .mHide();
              var $grpHide = self.$right
                .find("option")
                .closest("optgroup")
                .mHide();
              var $grpShow = self.$right
                .find("option:not(.hidden)")
                .parent("optgroup")
                .mShow();
            } else {
        ...
        self.$right.closest("form").on("submit", function(e) {
          ...
          self.$left
            .find("option")
            .prop("selected", self.options.submitAllLeft);
          self.$right
            .find("option")
            .prop("selected", self.options.submitAllRight);
        });
      ...
      moveToRight: function($options, event, silent, skipStack) {
        ...
        if (typeof self.callbacks.moveToRight == "function") {
          return self.callbacks.moveToRight(
            self,
            $options,
            event,
            silent,
        ...
        if (typeof self.callbacks.beforeMoveToRight == "function" && !silent) {
          if (
            !self.callbacks.beforeMoveToRight(self.$left, self.$right, $options)
          ) {
            return false;
          }
        }
        ...
        if (
          typeof self.callbacks.sort.right == "function" &&
          !silent &&
          !self.doNotSortRight
        ) {
      ...
      moveToLeft: function($options, event, silent, skipStack) {
        ...
        if (typeof self.callbacks.moveToLeft == "function") {
          return self.callbacks.moveToLeft(
            self,
            $options,
            event,
            silent,
        ...
        if (typeof self.callbacks.beforeMoveToLeft == "function" && !silent) {
          if (
            !self.callbacks.beforeMoveToLeft(self.$left, self.$right, $options)
          ) {
            return false;
          }
        }
      ...
      moveFromAtoB: function(
        ...
        if (typeof self.callbacks.moveFromAtoB == "function") {
          return self.callbacks.moveFromAtoB(
            self,
            $source,
            $destination,
            $options,
        ...
        $options.each(function(index, option) {
          ...
          if ($option.is("optgroup") || $option.parent().is("optgroup")) {
            var $sourceGroup = $option.is("optgroup")
              ? $option
              : $option.parent();
            var optgroupSelector =
              "optgroup[" +
              self.options.matchOptgroupBy +
              '="' +
              $sourceGroup.prop(self.options.matchOptgroupBy) +
              '"]';
      ...
      startUp: function($left, $right) {
        $right.find("option").each(function(index, rightOption) {
          if (
            ...
            $left
              .find(
                optgroupSelector + ' option[value="' + rightOption.value + '"]'
              )
              .each(function(index, leftOption) {
                leftOption.remove();
              });
            $left.find(optgroupSelector).removeIfEmpty();
          } else {
            var $option = $left.find(
  ...
  var isIE =
    ua.indexOf("MSIE ") + ua.indexOf("Trident/") + ua.indexOf("Edge/") > -3;
  ...
  $.fn.mSort = function(callback) {
    ...
    this.find("optgroup").each(function(i, group) {
      $(group)
        .children()
        .sort(callback)
        .appendTo(group);
