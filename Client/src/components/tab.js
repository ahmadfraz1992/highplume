...
class Tabs extends Component {
      ...
      tabs: [
        {
          ...
          panel: (
            <div>
              <p>
                Bacon ipsum dolor amet pork prosciutto tail ground round cow
                pancetta ham beef. Brisket cupim shoulder drumstick turkey
                sausage cow pork beef pig venison boudin. Ham hock bacon
                hamburger alcatra boudin shank shankle porchetta short ribs.
                Jowl shank shoulder, pork belly tail ham hock ribeye fatback
                sirloin doner beef swine ground round meatball hamburger.
              </p>
              <p>
                Venison pork turkey jerky pig. Kevin andouille pastrami, ham
                hock sausage landjaeger sirloin tri-tip spare ribs boudin
                kielbasa tenderloin bresaola. Short loin ribeye biltong capicola
                salami tenderloin, fatback ground round rump sirloin meatloaf
                porchetta. Pork loin alcatra short loin ham hock kevin salami
                beef ribs filet mignon leberkas. Bresaola pork landjaeger, tail
                jowl t-bone corned beef. Cupim ground round tail brisket, pork
                belly short loin t-bone. Beef ribs pork chop kevin short ribs
                frankfurter alcatra ball tip ground round jerky.
              </p>
            </div>
          )
        },
          ...
          panel: (
            <div>
              <p>
                ...
                tattered for solum oculi eorum defunctis go lum cerebro.
              </p>
            </div>
          )
        }
  ...
  render() {
    ...
    const activeIndex = tabs.indexOf(
      tabs.filter(tab => tab.id === activeId)[0]
    );
        ...
        <div className="tab-set">
          <Measure>
            {dimensions => (
              <Motion
                style={{
                  x: spring((dimensions.width / 3) * activeIndex, fastSpring)
                }}
              >
                {value => (
                  <AriaTabList className="tab-list">
                    {tabs.map(({ id, title }) => (
                      <AriaTab key={id} id={id} isActive={id === activeId}>
                        {(props, isActive) => (
                          <div
                            {...props}
          ...
          <div className="tab-panels">
            <ReactFluidContainer height="auto" style={{ overflow: "hidden" }}>
              <div>
                {tabs.map(({ id, panel }) => (
                  <AriaPanel
                    key={id}
